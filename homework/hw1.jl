...
[RGB(p.r, 0, 0) for p in philip_head]
...
function mean_color(image)
	mean_red = mean(p.r for p in image)
	mean_green = mean(p.g for p in image)
	mean_blue = mean(p.b for p in image)
...
# ╔═╡ 9754c3b7-e89a-4767-8b8d-dfc3bf83881f
RGB([0.5, 0, 0.2]...)
...
# ╔═╡ 62e7146c-7c75-4165-9983-2cef4c1a3f2f
function smooth_step(x::Number)
...
function custom_filter(pixel::AbstractRGB)
	...
	return RGB(smooth_step(pixel.r), smooth_step(pixel.g), smooth_step(pixel.b))
end
